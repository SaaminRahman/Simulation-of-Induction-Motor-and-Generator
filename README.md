...
Because the system is self-excited, the simulation uses normalized/relative currents based on the admittances of the circuit to represent their relative magnitudes and phase angles:
1. **Frequency Scaling:** All reactive components ($X_1, X_2, X_c, X_L, X_m$) are scaled by the operating frequency ratio: $\frac{f}{f_{base}}$ (Finding the operating frequency is discussed later).
...
* The magnetic field contribution of each phase is a vector directed perpendicular to the physical coil orientation, with a magnitude proportional to the instantaneous current.
...
* The electrical frequency of the rotor currents is the slip frequency: $\omega_r = \omega \cdot s$.
* The induced current in each bar is produced due to the induced voltage which is created due to relative motion of the magnetization magnetic field and the rotor bars. At t=0, the velocity vector of the bars are at different angle with the magnetization magnetic field. That angles act as initial phase shift for induced voltage in different bar. The frequency of the induced voltage can be found from the relative velocity of a bar with respect to the magnetization magnetic field.
* Current lags the voltage by  rotor power factor angle $\theta_{pf} = \arctan(\frac{sX_2}{R_2})$ .
* Each bar creates a magnetic field vector perpendicular to its position.
* The total **Rotor Field ($B_r$)** is the complex vector sum of the magnetic fields produced by all individual rotor bars.
...
1. **Objective:** Find the frequency $f$ where the **real part of the total admittance**  $Re(Y)$  is exactly zero. (This represents the real power balance equation). Total admittance is equal to the sum of admittance of rotor branch, magnetizing branch and stator branch. 
...
In a self-excited induction generator, steady-state operation requires both real and reactive power balance. The magnetizing reactance $X_m$ adjusts itself (due to magnetic saturation) to satisfy the reactive power balance.
